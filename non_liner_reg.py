...
from functions import mean_squared_error, sigmoid
...
def get_data(data_size, train_size=None, train_ratio=0.9, start=-5.0, end=5.0):
    ...
    t_all += np.random.normal(0, 2, data_size)  # ちょっとだけノイズをたす
    # 全データ作成、以上

    # x_train = x_all[:train_size]  # 全体データのうち、train_size分は教師データ
    # t_train = t_all[:train_size]
    #
    # x_test = x_all[train_size:]  # 全体データのうち train_size分以外はテストに使うデータ
...
def main(args):
    ...
    data_size = 10000  # 母集団のデータ数
    train_size = data_size - 100  # うち教師データ数

    (x_train, t_train), (x_test, t_test) = get_data(data_size, train_size)
    ...
    for i in range(iteration_count):  # batch_sizeコごとのバッチを、iteration_count 回繰り返す
        ...
        if i != 0 and i % 10000 == 0:
            learning_rate *= 0.5
            print(f'学習率変更| {learning_rate:.8f}')
